    ...
    <div className="flex flex-col gap-5">
      <div className="flex items-center gap-2.5">
        <RangeFilter
          ...
          options={[1, 7, 30]}
          labels={{ 1: t("perfPanel.range24h"), 7: t("perfPanel.range7d"), 30: t("perfPanel.range30d") }}
        />
        <button
          type="button"
          onClick={() => {
        ...
        >
          {t("perfPanel.groupByModel")}
        </button>
        <input
          type="text"
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder={t("perfPanel.searchPlaceholder")}
      ...
      {perf.isPending && (
        <div className="flex h-64 items-center justify-center text-sm text-fg-3">{t("perfPanel.loadingData")}</div>
      )}
