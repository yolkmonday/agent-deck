      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div className="flex flex-col gap-1">
            <span className="text-base font-semibold">{t("modelPickerDialog.title")}</span>
            <span className="text-xs text-fg-2">
              {t(fetched.length === 1 ? "modelPickerDialog.returnedCount.one" : "modelPickerDialog.returnedCount.other", {
                n: fetched.length,
              })}
            </span>
          </div>
        ...
        <div className="flex items-center gap-2 rounded-md border border-border bg-bg px-2.5 py-1.75">
          ...
          <input
            ...
            placeholder={t("modelPickerDialog.searchPlaceholder")}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto rounded-[10px] border border-border bg-bg px-3 py-2">
          {rows.length === 0 ? (
            <div className="py-10 text-center text-sm text-fg-3">{t("modelPickerDialog.noMatch")}</div>
          ) : (
            <div className="flex flex-col">
              <label className="ad-interactive flex cursor-pointer items-center gap-2.5 border-b border-border pb-2.5 text-[11.5px] text-fg-3 hover:bg-surface-2">
                ...
                />
                {t("modelPickerDialog.selectAllVisible")}
              </label>
              {rows.map(({ id }) => {
                ...
                return (
                  ...
                  >
                    ...
                    {already && <span className="shrink-0 text-[11px] text-fg-3">{t("modelPickerDialog.alreadyAdded")}</span>}
                  </label>
                );
              })}
            </div>
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between gap-3">
            <span className="text-xs text-fg-2">
              {t("modelPickerDialog.selectedOf", { selected: selected.length, total: fetched.length })}
            </span>
            <div className="flex items-center gap-2">
              ...
              >
                {t(selected.length === 1 ? "modelPickerDialog.addCount.one" : "modelPickerDialog.addCount.other", {
                  n: selected.length,
                })}
              </button>
            </div>
